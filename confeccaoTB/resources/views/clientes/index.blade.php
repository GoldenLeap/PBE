                                ...
                                @forelse ($clientes as $cliente)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/20">
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            ...
                                            <form id="del-{{ $cliente->id }}" action="{{ route('clientes.destroy', $cliente->id) }}" method="POST" class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                            <button type="button" onclick="openDeleteModal('del-{{ $cliente->id }}', '{{ addslashes($cliente->nome) }}')" class="text-red-500 hover:text-red-700 text-sm font-semibold ml-2">
                                                Excluir
                                            </button>
                                        </td>
                                    </tr>
    ...
    <div id="delete-modal" style="display:none; position:fixed; inset:0; z-index:9999; align-items:center; justify-content:center;">
        ...
        <div style="position:relative; background:white; border-radius:12px; padding:24px; width:100%; max-width:400px; margin:0 16px; box-shadow:0 20px 60px rgba(0,0,0,0.3);">
            <div style="text-align:center; margin-bottom:16px;">
                ...
                <p style="font-size:14px; color:#6b7280;">Tem certeza que deseja excluir este cliente? Esta ação não poderá ser desfeita.</p>
            </div>
